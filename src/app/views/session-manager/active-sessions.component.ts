...
export class ActiveSessionsComponent implements OnInit {
  ...
  public isKillWebfig: boolean = false;
  ...
  confirmKillSession(sessionId: string, isWebfig: boolean = false) {
    this.killSessionId = sessionId;
    this.isKillWebfig = isWebfig;
    this.killSessionVisible = true;
  }

  executeKillSession() {
    if (this.isKillWebfig) {
      this.http.post(`/api/proxy/${this.killSessionId}/close`, {}).subscribe({
        next: () => {
          this.show_toast('Success', 'WebFig session terminated', 'success');
          this.loadSessions();
          this.killSessionVisible = false;
        },
        error: () => {
          this.show_toast('Error', 'Failed to terminate WebFig session', 'danger');
          this.killSessionVisible = false;
        }
      });
    } else {
  ...
  generateShareLink() {
    ...
    const sharePromise = this.isWebfigSession
      ? this.data_provider.shareWebfigSession(this.shareSessionId, this.shareRole, this.shareName, this.shareType, targetUserId, this.sharePassword, this.shareOneTime, this.shareHostApproval)
      : this.data_provider.shareSession(this.shareSessionId, this.shareRole, this.shareName, this.shareType, targetUserId, this.sharePassword, this.shareOneTime, this.shareHostApproval);
